...
#if defined(FEATURE_MEA_BIDI)
            //BIDI in HTML composer
            if(settings->editableSupportEnabled() && dir != RTL && o->isText()) {
                if(toRenderText(o)->checkIsRTL()) {
        ...
        if (o->isFloatingOrPositioned()) {
		  ...
		  // SAMSUNG CHANGE <<
                ...
                if ((breakAll || breakWords) && !midWordBreak) {
                    wrapW += charWidth;
		      // SAMSUNG CHANGE
		      charWidth = textWidth(t, pos, 1 + extendedCharCount, f, w + wrapW, isFixedPitch, collapseWhiteSpace);
                ...
                if (betweenWords || midWordBreak) {
                    ...
                    if (wordTrailingSpaceWidth && currentCharacterIsSpace)
                        ...
                        additionalTmpW = textWidth(t, lastSpace, pos + extendedCharCount - lastSpace, f, w + tmpW, isFixedPitch, collapseWhiteSpace) + lastSpaceWordSpacing;
                    tmpW += additionalTmpW;
				...
				// SAMSUNG CHANGE
                            int charWidth = textWidth(t, pos, 1 + extendedCharCount, f, w + tmpW, isFixedPitch, collapseWhiteSpace) + (applyWordSpacing ? wordSpacing : 0);
