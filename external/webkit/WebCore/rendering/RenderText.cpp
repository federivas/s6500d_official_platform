...
//GGSM_SEL_MEA_BIDI - Start
#if defined(FEATURE_MEA_BIDI) //FUNCTION_MEA_BIDI
RefPtr<StringImpl> handleArabicString(RefPtr<StringImpl> str); //Prototype
#endif
//GGSM_SEL_MEA_BIDI - End
...
//GGSM_SEL_MEA_BIDI - Start
#if defined(FEATURE_MEA_BIDI) //FUNCTION_MEA_BIDI
    m_text = handleArabicString(m_text);
#endif
//GGSM_SEL_MEA_BIDI - End
...
RefPtr<StringImpl> RenderText::handleArabicString(RefPtr<StringImpl> str)
{
    int nSrcLen = str->length();
    const UChar * srsStr = str->characters();			
    ...
    RefPtr<StringImpl> returnStr = StringImpl::create (tmpBuff, nSrcLen);
    free(tmpBuff);
    return returnStr;
}

//BIDI in HTML composer - start
bool RenderText::applyRTL(RefPtr<StringImpl> text)
{
    ...
    if(applyRTLForString((unsigned short *)tmpBuff, length))
    {
        result = true;
    }
