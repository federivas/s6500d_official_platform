...
void InlineTextBox::paint(RenderObject::PaintInfo& paintInfo, int tx, int ty)
{
    ...
    if (paintInfo.phase == PaintPhaseForeground) {
        ...
        if (useCustomUnderlines) {
            Vector<CompositionUnderline>& underlines = renderer()->document()->frame()->editor()->customCompositionUnderlines();
...
void InlineTextBox::paintCompositionUnderline(GraphicsContext* ctx, int tx, int ty, CompositionUnderline& underline)
{
    tx += m_x;
    ty += m_y;

__android_log_print(ANDROID_LOG_DEBUG, "Paint ", "InlineTextBox::paintCompositionUnderline %d" , ctx);
    ...
    int start = 0;                 // start of line to draw, relative to tx
    int width = m_width;           // how much line to draw
 __android_log_print(ANDROID_LOG_DEBUG, "Inline ", " Inline start %d m_width %d" , start ,  width  );
    ...
    if (m_truncation != cNoTruncation) {
	__android_log_print(ANDROID_LOG_DEBUG, "Inline ", " m_truncation != cNoTruncation "  );
    ...
    if (!useWholeWidth) {
	 ...
	 __android_log_print(ANDROID_LOG_DEBUG, "Inline ", " Inline start %d toRenderText width %d" , start ,  width  );
    }
    ...
    {
     		SkScalar left , right , top , bottom; 
  
		
		left = SkIntToScalar(tx + start);
		if(underline.isHighlightColor)
			top = SkIntToScalar(ty);
		else 
			top = SkIntToScalar(ty + height() - lineThickness + 1);

		right =  left + SkIntToScalar(width);
		
	
		
		if(underline.isHighlightColor)
			bottom = top + SkIntToScalar(height());
		else 
			bottom = top + SkIntToScalar(lineThickness);
		
		SkRect tempRect;
//GGSM_SEL_MEA_BIDI - Start
//FUNCTION_MEA_BIDI - Start
#if defined(FEATURE_MEA_BIDI)
		//RTL underline handling
		if (direction() == RTL)
		{
			const int correctionOffset = 8;
			tempRect.set(m_x+correctionOffset , top , m_x+width+correctionOffset , bottom);
		}
		else
		{
...
#else
//FUNCTION_MEA_BIDI - End
//GGSM_SEL_MEA_BIDI - End
		tempRect.set(left , top , right , bottom);
#endif

		if(renderer()->document()->frame()->editor()->getUnderLineUpdateFlag())
		{
			underline.contentDrawRect.append(tempRect);
		}
		
		__android_log_print(ANDROID_LOG_DEBUG, "Inline ", " Inline  left:%f top:%f right:%f bot:%f " , left , top , right , bottom );

		//renderer()->document()->frame()->editor()->underlineRect.append(contentRect); 			
    }
    else 
    {				
    start += 1;
    width -= 2;
    	...
    	//ANDROID_LOG_PRINT(ANDROID_LOG_DEBUG, "Inline ", " Inline start %d width %d" , start ,  width  );
    	__android_log_print(ANDROID_LOG_DEBUG, "Inline ", " Inline start %d width %d" , start ,  width  );
