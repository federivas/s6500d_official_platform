...
#include "RenderImage.h" //SAMSUNG CHANGE
#include "Text.h"
...
//+HTML_COMPOSER
String Range::toStringForEditing(ExceptionCode& ec) const
{
    if (!m_start.container()) {
        ec = INVALID_STATE_ERR;
        return String();
	...
	n->showTreeForThis();
        ...
        else if( n->renderer() && n->renderer()->isBR() ) {
            result.append("\n", 1);
        }
    }
...
//SAMSUNG CHANGE >>
IntRect Range::boundingBoxEx(bool *bHasImage)
{
    IntRect result;
    Vector<IntRect> rects;
    ...
    if ( startContainer && endContainer) {
        ...
        for (Node* node = firstNode(); node != stopNode; node = node->traverseNextNode()) {
            ...
            if (r && r->isText()) {
                ...
                int startOffset = node == startContainer ? m_start.offset() : 0;
                int endOffset = node == endContainer ? m_end.offset() : numeric_limits<int>::max();
                bool includeBr = r->isBR(); // SAMSUNG CHANGE
                renderText->absoluteRectsForRange(rects, startOffset, endOffset, useSelectionHeight, includeBr); // SAMSUNG CHANGE
            }
            else if (r && r->isImage()) {
                ...
                rects.append(r);
				// To know whether selection has atleast an image to prevent clearing selection when there is no text at all
				if (bHasImage) {
					*bHasImage = true;
				}
            }
