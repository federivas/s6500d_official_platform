...
#if _SISO_CSS_OPT_
    typedef HashMap<std::pair<AtomicStringImpl*, AtomicStringImpl*>, CSSRuleDataList* > AtomDescRuleMap;
    // Add the Descendent rules to the list
    void addToDescRuleSet(AtomicStringImpl* key1, AtomicStringImpl* key2, AtomDescRuleMap& map, CSSStyleRule* rule, CSSSelector* sel);
    //Get the class descendent rules
    CSSRuleDataList* getDescClassRules(AtomicStringImpl* key1, AtomicStringImpl* key2, unsigned int index) {
        return m_descClassRules[index].get(std::pair<AtomicStringImpl*, AtomicStringImpl*>(key1, key2));
    }
    //Get the id descendent rules
    CSSRuleDataList* getDescIDRules(AtomicStringImpl* key1, AtomicStringImpl* key2, unsigned int index) {
        return m_descIDRules[index].get(std::pair<AtomicStringImpl*, AtomicStringImpl*>(key1, key2));
    }
    //Get the tag descendent rules
    CSSRuleDataList* getDescTagRules(AtomicStringImpl* key1, AtomicStringImpl* key2, unsigned int index) {
        return m_descTagRules[index].get(std::pair<AtomicStringImpl*, AtomicStringImpl*>(key1, key2));
    }
#endif
...
#if _SISO_CSS_OPT_
    AtomDescRuleMap m_descClassRules[3];
    AtomDescRuleMap m_descIDRules[3];
    AtomDescRuleMap m_descTagRules[3];
#endif
...
#if _SISO_CSS_OPT_
    /* tag descendant rules => get all the rules which are associated to
       all the ancestor meember elements of the current element */
    Element* e = m_element;
    while (true) {
        Node* n = e->parentNode();
        if (!n || !n->isElementNode())
            ...
            //id
            if (m_element->hasID()) {
                const AtomicString& localName2 = m_element->getIDAttribute();
                matchDescRulesForList(e,rules,localName2.impl(),0,firstRuleIndex, lastRuleIndex);
            }
            //class
            if(m_element->hasClass()){
                ASSERT(m_styledElement);
                const SpaceSplitString& classNames2 = m_styledElement->classNames();
                size_t size2 = classNames2.size();
                for (size_t i = 0; i < size2; ++i){
                    const AtomicString& localName2 = classNames2[i];
                    matchDescRulesForList(e,rules,localName2.impl(),1,firstRuleIndex, lastRuleIndex);
                }
            }
            //tag
            const AtomicString& localName2 = m_element->localName();
            matchDescRulesForList(e,rules,localName2.impl(),2,firstRuleIndex, lastRuleIndex);
        }
    }
#endif
...
#if _SISO_CSS_OPT_
        unsigned tpos = m_matchedRules[0]->position();
#endif
        addMatchedDeclaration(m_matchedRules[0]->rule()->declaration());
        for (unsigned i = 1; i < m_matchedRules.size(); i++) {
#if _SISO_CSS_OPT_
            // To avoid matched rules duplication after optimization.
            if(tpos!= m_matchedRules[i]->position()) {
#endif
            addMatchedDeclaration(m_matchedRules[i]->rule()->declaration());
                ...
                lastRuleIndex--;
            tpos= m_matchedRules[i]->position();
#endif
        }
...
#if _SISO_CSS_OPT_
void CSSStyleSelector::matchDescRulesForList(Element* e,CSSRuleSet* rules, AtomicStringImpl* localNameImpl, unsigned int index, int& firstRuleIndex, int& lastRuleIndex)
{
    //[ rule e.g. : #id .class2 {}]
    if(e->hasID()){
        const AtomicString& localName = e->getIDAttribute();
        matchRulesForList(rules->getDescIDRules(localName.impl(), localNameImpl,index), firstRuleIndex, lastRuleIndex);
    }
    //[ rule e.g. : .class1 .class2 {}]
    if(e->hasClass()){
        const SpaceSplitString& classNames = static_cast<StyledElement*>(e)->classNames();
        size_t size1 = classNames.size();
        for (size_t j = 0; j < size1; ++j){
            const AtomicString& localName = classNames[j];
            matchRulesForList(rules->getDescClassRules(localName.impl(), localNameImpl,index), firstRuleIndex, lastRuleIndex);
        }
    }
    //[ rule e.g. : div .class2 {}]
    const AtomicString& localName = e->localName();
    matchRulesForList(rules->getDescTagRules(localName.impl(), localNameImpl,index),firstRuleIndex, lastRuleIndex);
}
#endif
...
{ 
    ...
    for(unsigned k=0;k<3;k++){
        deleteAllValues(m_descClassRules[k]);
        deleteAllValues(m_descIDRules[k]);
        deleteAllValues(m_descTagRules[k]);
    }
#endif
...
#if _SISO_CSS_OPT_
void CSSRuleSet::addToDescRuleSet(AtomicStringImpl* key1, AtomicStringImpl* key2, AtomDescRuleMap& map,CSSStyleRule* rule, CSSSelector* sel)
{
    if (!key1 || !key2) return;
    CSSRuleDataList* rules = map.get(std::pair<AtomicStringImpl*, AtomicStringImpl*>(key1, key2));
    if (!rules) {
        rules = new CSSRuleDataList(m_ruleCount++, rule, sel);
        map.set(std::pair<AtomicStringImpl*, AtomicStringImpl*>(key1, key2), rules);
    } else
        rules->append(m_ruleCount++, rule, sel);
}
#endif
...
{
#if _SISO_CSS_OPT_
    // Optimization method for CSS Descendant rules for not-matched cases in the 1st level
    //id
    if (sel->m_match == CSSSelector::Id) {
        CSSSelector::Relation relation = sel->relation();
        CSSSelector* selL1 = sel; //selector level1
        selL1 = selL1->tagHistory();
        if (selL1){
            // If the rule has any selectors
            if(relation==CSSSelector::Descendant){

                if (selL1->m_match == CSSSelector::Id){
                    addToDescRuleSet(selL1->m_value.impl(), sel->m_value.impl(), m_descIDRules[0], rule, sel);
                    return;
                }
                if (selL1->m_match == CSSSelector::Class){
                    addToDescRuleSet(selL1->m_value.impl(), sel->m_value.impl(), m_descClassRules[0], rule, sel);
                    return;
                }
                const AtomicString& localName = selL1->m_tag.localName();
                addToDescRuleSet(localName.impl(), sel->m_value.impl(), m_descTagRules[0], rule, sel);
                return;
            }
        }
        // Add rest of the rules
    ...
    if (sel->m_match == CSSSelector::Class) {
        ...
        if (selL1){
            ...
            if(relation==CSSSelector::Descendant){
                ...
                if (selL1->m_match == CSSSelector::Id){
                    addToDescRuleSet(selL1->m_value.impl(), sel->m_value.impl(), m_descIDRules[1], rule, sel);
                    return;
                }
                if (selL1->m_match == CSSSelector::Class){
                    addToDescRuleSet(selL1->m_value.impl(), sel->m_value.impl(), m_descClassRules[1], rule, sel);
                    return;
                }
                const AtomicString& localName = selL1->m_tag.localName();
                addToDescRuleSet(localName.impl(), sel->m_value.impl(), m_descTagRules[1], rule, sel);
                return;
            }
        }
        // Add rest of the rules
    ...
    //tag
    const AtomicString& localNameSel = sel->m_tag.localName();
    if (localNameSel != starAtom) {
        CSSSelector::Relation relation = sel->relation();
        CSSSelector* selL1 = sel; //selector level1
        selL1 = selL1->tagHistory();
        if (selL1){
            ...
            if(relation==CSSSelector::Descendant){
                ...
                if (selL1->m_match == CSSSelector::Id){
                    addToDescRuleSet(selL1->m_value.impl(), localNameSel.impl(), m_descIDRules[2], rule, sel);
                    return;
                }
                if (selL1->m_match == CSSSelector::Class){
                    addToDescRuleSet(selL1->m_value.impl(), localNameSel.impl(), m_descClassRules[2], rule, sel);
                    return;
                }
                const AtomicString& localName = selL1->m_tag.localName();
                addToDescRuleSet(localName.impl(), localNameSel.impl(), m_descTagRules[2], rule, sel);
                return;
            }
        }
        // Add rest of the rules
        addToRuleSet(localNameSel.impl(), m_tagRules, rule, sel);
        return;
    }
#else
...
// length
    ...
    case CSSPropertyWebkitColumnRuleWidth:
    {
        ...
        case CSSValueInvalid:
            //SAMSUNG FIX >>
            {
                double check_width = primitiveValue->computeLengthDouble(style(), m_rootElementStyle, zoomFactor);
                if ((check_width>0.0) && (check_width<1.0) && (m_element->hasTagName(tableTag) || m_element->hasTagName(tdTag) || m_element->hasTagName(thTag)))
                    width=1;
                else
                    width=static_cast<short>(check_width);
            }
