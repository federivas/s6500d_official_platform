...
void device_probe_drivers(struct btd_device *device, GSList *profiles)
{
	...
	if (device->blocked) {
		DBG("Skipping drivers for blocked device");// [Security Log] %s", addr);
		goto add_uuids;
	}

	DBG("Probing drivers");// [Security Log] for %s", addr);
	...
	for (list = device_drivers; list; list = list->next) {
		...
		if (err < 0) {
			error("%s driver probe failed for device", driver->name);
...
static void device_remove_drivers(struct btd_device *device, GSList *uuids)
{
	...
	DBG("Removing drivers");// [Security Log] for %s", dstaddr);
	...
	for (list = device->drivers; list; list = next) {
		...
		for (uuid = driver->uuids; *uuid; uuid++) {
			...
			DBG("UUID %s was removed from device", *uuid);
...
static void search_cb(sdp_list_t *recs, int err, gpointer user_data)
{
	...
	if (err < 0) {
		error("error updating services: %s (%d)", strerror(-err), -err);
		goto send_reply;
	}
						...
						gboolean oob)
{
	...
	DBG("Temporary agent registered at %s", agent_path);
...
static void create_bond_req_exit(DBusConnection *conn, void *user_data)
{
	...
	ba2str(&device->bdaddr, addr);
	DBG("requestor exited before bonding was completed");
						...
						uint32_t passkey, void *cb)
{
	...
	ba2str(&device->bdaddr, addr);
	DBG("Requesting agent authentication");

	if (device->authr) {
		error("Authentication already requested");
		return -EALREADY;
	}
...
void device_cancel_authentication(struct btd_device *device, gboolean aborted)
{
	...
	ba2str(&device->bdaddr, addr);
	DBG("Canceling authentication request");
