...
struct btd_adapter {
	...
	GSList *audio_devices;	
};

static void adapter_set_pairable_timeout(struct btd_adapter *adapter,
					guint interval);

void update_audio_devices(struct btd_device *dev, GSList *devices)
{
	struct btd_adapter *adapter = device_get_adapter(dev);
	adapter->audio_devices = devices;
}

void update_link_policy(struct btd_adapter *adapter)
{
	GSList *audio_devices = adapter->audio_devices;
	...
	pthread_mutex_lock(&mutex_audio_devices);	
	for (l = audio_devices; l != NULL; l = l->next) {
		dev = l->data;
		if ((dev != NULL) && (dev->is_streaming == TRUE)) {
			DBG("Sink is streaming");			
			is_streaming = TRUE;
			break;
		}
	}
	pthread_mutex_unlock(&mutex_audio_devices);	
	if (is_streaming == FALSE) {
		DBG("There is no sink in streaming state");
		return;
	}
	if (!adapter)
	  return;
	...
	bacpy(&cr->bdaddr, &dev->dst);
	cr->type = ACL_LINK;
	if (ioctl(dd, HCIGETCONNINFO, (unsigned long) cr) < 0) {
	  DBG("Get connection info failed");
	  goto RET_FAIL;
	}

	if (!(cr->conn_info->link_mode& HCI_LM_MASTER)) {
		DBG("Switching to Master role");
	    if (hci_switch_role(dd, &dev->dst, 0, 5000) < 0) {
		   	DBG("Switch role request failed");
			goto RET_FAIL;
	    }
	}
	DBG("Done with role switch");

	if (hci_write_link_policy(dd, htobs(cr->conn_info->handle),
		0, 1000) < 0) {
		DBG("HCI write_link_policy_settings request failed");
		goto RET_FAIL;
	}
	DBG("Updated link policy settings as 0");
...
gboolean adapter_init(struct btd_adapter *adapter)
{
	...
	pthread_mutex_init(&mutex_audio_devices, NULL);
					...
					void *user_data)
{
 ...
 char value[PROPERTY_VALUE_MAX];	// Samsung Bluetooth feature - IT Policy
	...
	if (device_is_trusted(device) == TRUE && !(strcmp(value, "1") == 0)) { // Samsung Bluetooth feature - IT Policy
