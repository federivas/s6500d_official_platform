...
void avdtp_update_link_policy(struct avdtp *session)
{
	...
	if (hci_write_link_policy(dd, htobs(cr->conn_info->handle), 0x5, 1000) < 0){
...
static void stream_free(struct avdtp_stream *stream)
{
	...
	avdtp_update_link_policy(stream->session);
				...
				struct seid_req *req, unsigned int size)
{
	...
	avdtp_update_link_policy(session);
