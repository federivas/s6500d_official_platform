...
static int avrcp_key = 0; // 0: idle 1:FF 2: rewind  
//
...
static sdp_record_t *avrcp_tg_record()
{
	...
	uint16_t avrcp_ver = 0x0100, avctp_ver = 0x0103, feat = 0x000f; // samsung bluetooth feature
...
// samsung bluetooth feature    
  if(pressed == 1){
   if(key_map[i].uinput == KEY_FASTFORWARD)
    avrcp_key = 1;
   if(key_map[i].uinput == KEY_REWIND)
    avrcp_key = 2;
  }else{
   avrcp_key = 0;
  }
  DBG("AVRCP: avrcp_key : %d",avrcp_key);
//
...
//samsung bluetooth feature
 if(event_id ==0x01)
 { if(avrcp_key == 1)
     event_data = STATUS_FWD_SEEK;
   else if(avrcp_key ==2)
     event_data = STATUS_REV_SEEK;
 }
 //
 DBG("Notification data is %d %d", (int)event_id, (int)event_data);
								...
								void *data)
{
 ...
 //samsung bluetooth feature
  if(avrcp_key == 1)
    play_status = STATUS_FWD_SEEK;
  else if(avrcp_key ==2)
    play_status = STATUS_REV_SEEK;
  //
